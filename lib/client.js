...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// recent detail
			".u_days{flex-direction:column;display:flex}",
			...
			".u_dayDate{color:var(--dsw-alias-label-secondary);flex:none;width:64px;font-size:12px;line-height:18px;font-variant-numeric:tabular-nums}",
			".u_dayTokens{color:var(--dsw-alias-label-primary);flex:none;font-size:12px;line-height:18px;font-variant-numeric:tabular-nums}",
			".u_dayHit{color:var(--dsw-alias-label-tertiary);flex:none;width:52px;font-size:11px;line-height:18px;font-variant-numeric:tabular-nums;text-align:right}",
			".u_dayBar{background:var(--u-accent,#1f6feb);border-radius:2px;height:6px;flex:1;min-width:4px;opacity:.65}",
											...
											children: recent.map((day) => {
												...
												return react_jsx_runtime.jsxs("button", {
													...
													children: [
														...
														react_jsx_runtime.jsx("div", { className: "u_dayBar", style: { width: `${share}%` } })
													]
												}, day.date);
											})
										});
