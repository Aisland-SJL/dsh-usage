...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".u_day{width:100%;min-height:30px;align-items:center;gap:8px;border:0;background:0 0;border-bottom:1px solid var(--dsw-alias-border-l1);padding:5px 0;font:inherit;text-align:left;cursor:pointer;display:flex}",
			".u_day:last-child{border-bottom:0}",
			".u_day:hover{background:var(--dsw-alias-interactive-bg-hover)}",
			".u_dayDate{color:var(--dsw-alias-label-secondary);flex:none;width:56px;font-size:12px;line-height:18px;font-variant-numeric:tabular-nums;text-align:left}",
			".u_dayHit{color:var(--dsw-alias-label-tertiary);flex:none;width:58px;font-size:11px;line-height:18px;font-variant-numeric:tabular-nums;text-align:right}",
			".u_dayTokens{color:var(--dsw-alias-label-primary);flex:none;width:66px;font-size:12px;line-height:18px;font-variant-numeric:tabular-nums;text-align:right}",
			".u_dayBarTrack{flex:none;width:60px;height:6px;border-radius:3px;background:var(--dsw-alias-fill-l2);overflow:hidden}",
			".u_dayBar{background:var(--u-accent,#1f6feb);border-radius:inherit;height:100%;min-width:3px;opacity:.75}",
		...
		function UsagePanel({ wide, t }) {
			...
			react.useEffect(() => {
				...
				const measure = () => {
					...
					if (width > 0) setDockMetrics({ left: rect.left + padLeft + margin, width, centerX: rect.left + rect.width / 2 });
					else setDockMetrics({ left: 14, width: 0, centerX: rect.left + rect.width / 2 });
				};
				measure();
					...
					children: [
						...
						react_jsx_runtime.jsx("button", {
							...
							onClick: () => {
								...
								// panel's bottom-left corner to the gear's current
								// position (measured on every open).
								if (open) {
									setOpen(false);
									return;
								}
